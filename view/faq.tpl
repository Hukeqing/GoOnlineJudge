...
<p>如果没有特别说明，你的程序应该从标准输入（stdin，传统意义上的“键盘”）读入，并输出到标准输出（stdout，传统意义上的“屏幕”），不要使用文件做输入输出。由于系统是在你的程序运行结束后开始检查输出是否是正确的，对于有多组测试数据的输入，可以全部读入之后再输出，也可以处理一组测试数据就输出一组。<strong>另外友情提醒，尽量减少c和c++的读入的混用，有可能会产生不必要的读入错误，有可能会引发一系列其他错误。若使用c++读入但不使用c读入的情况下，可以关闭同步来加快读入速度。</strong></p>
<br>
...
<p>Java和Python代码的时间和内存限制一般是其他编程语言的两倍</p>
<br>
